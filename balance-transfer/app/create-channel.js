...
var createChannel = function(channelName, channelConfigPath, orgName) {
	...
	return helper.getOrgAdmin(orgName).then((admin) => {
		...
		logger.error('Failed to enroll admin user for the organization \''+orgName+'\'. Error: ' + err);
		throw new Error('Failed to enroll admin user for the organization \''+orgName+'\'' + err);
	}).then((response) => {
