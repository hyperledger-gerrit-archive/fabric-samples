...
getEndorsers() {
	...
	do
		...
		endpoint=$(echo $peer_map | jq -r '.'"$ENDORSER_MSPID"'')
...
# Get the endorsers for this swap
echo "Retrieving swap endorsers"
getEndorsers

# Calculate the net payment due
echo "Calculate payment information"
calculatePayment

# Mark the payment as being settled
