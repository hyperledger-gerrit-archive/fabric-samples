...
instantiateChaincode() {
	...
	peer chaincode instantiate -o irs-orderer:7050 -C $CHANNEL_NAME -n irscc -l golang -v 0 -c '{"Args":["init","auditor","100000","rr_provider","libor"]}' -P "AND(OR('partya.peer','partyb.peer','partyc.peer'), 'auditor.peer')"
	echo "===================== Chaincode instantiated ===================== "
}

setReferenceRate() {
	CORE_PEER_LOCALMSPID=rr_provider
	CORE_PEER_ADDRESS=irs-rr_provider:7051
	CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/rr_provider.example.com/users/Admin@rr_provider.example.com/msp
	echo "===================== Invoke chaincode ===================== "
	peer chaincode invoke -o irs-orderer:7050 -C $CHANNEL_NAME --waitForEvent -n irscc --peerAddresses irs-rr_provider:7051 -c '{"Args":["setReferenceRate","libor","3"]}'
