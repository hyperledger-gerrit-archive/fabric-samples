...
auditor as well as an endorsement from a swap participant.
...
The key for the swap is a unique identifier combined with a common prefix that
identifies swap entries in the KVS namespace. Upon creation the key-level
...
KEY      | VALUE
---------|-----------------------------------------------------
swp1     | {StartDate: 2018-10-01, ..., ReferenceRate: "libor"}
pay1     | nil
rr_libor | 0.27
```
In this example, the swap with ID 1 is represented by the `swp1` and `pay1` KVS
entries. The reference rate is set to `libor`, which will cause the chaincode
 ...
 * `settlePayment(swapID)` - set the payment entry for the given swap ID to nil.
