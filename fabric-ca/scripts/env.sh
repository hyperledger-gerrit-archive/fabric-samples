...
function waitPort {
   ...
   if [ $result -ne 0 ]; then
      cat /tmp/apt-get.log
      fatal "Failed invoking 'apt-get update'"
   fi
   apt-get --assume-yes install netcat > /tmp/apt-get.log 2>&1
   result=$?
   if [ $result -ne 0 ]; then
      cat /tmp/apt-get.log
      fatal "Failed invoking 'apt-get install netcat'"
   fi
   if [ $# -ne 5 ]; then
      fatal "Usage: waitPort: $*"
   fi
   local what=$1
   local secs=$2
   local logFile=$3
