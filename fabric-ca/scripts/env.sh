...
SETUP_SUCCESS_FILE=/$SHAREDIR/setup.successful
...
# The TLS network trust file which contains the trust chains for all CAs in the network
TLS_NETWORK_TRUSTFILE=/${SHAREDIR}/tls-network.pem
...
function initOrdererVars {
   ...
   export ORDERER_GENERAL_TLS_ROOTCAS=[$TLS_NETWORK_TRUSTFILE]
}
...
function initPeerVars {
   ...
   export CORE_PEER_TLS_ROOTCERT_FILE=$TLS_NETWORK_TRUSTFILE
}
