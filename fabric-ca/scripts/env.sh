...
function initOrgVars {
   ...
   INT_CA_NAME=ica-${ORG}
   ROOT_CA_ADMIN_USER=rca-admin${ORG}
   INT_CA_ADMIN_USER=ica-admin${ORG}
   ...
   MSPDIR=/${SHAREDIR}/orgs/${ORG}/msp
   ANCHOR_TX_FILE=/${SHAREDIR}/orgs/${ORG}/anchors.tx
   # If ADMINCERTS is true, "admincerts must be in the SHARED volume;
   # but when using attributes to identity admin users, this is not necessary.
   ADMIN_MSP=/orgs/$ORG/admin/msp
   if $ADMINCERTS; then
      ADMIN_MSP=/${SHAREDIR}${ADMIN_MSP}
   fi
}
...
function initOrdererVars {
   ...
   export ORDERER_GENERAL_LOCALMSPID=$ORGMSP
...
function enrollAdmin {
   ...
   if [ ! -f $CORE_PEER_MSPCONFIGPATH ]; then
      ...
      export FABRIC_CA_CLIENT_HOME=orgs/$ORG/admin
      export FABRIC_CA_CLIENT_TLS_CERTFILES=$INT_CA_CHAINFILE
      fabric-ca-client enroll -u https://$ADMIN_NAME:$ADMIN_PASS@$INT_CA_HOST:7054 -M $CORE_PEER_MSPCONFIGPATH
   fi
}
...
function enrollUser {
   ...
   if [ ! -f $CORE_PEER_MSPCONFIGPATH ]; then
      ...
      export FABRIC_CA_CLIENT_TLS_CERTFILES=$INT_CA_CHAINFILE
      fabric-ca-client enroll -u https://$USER_NAME:$USER_PASS@$INT_CA_HOST:7054 -M $CORE_PEER_MSPCONFIGPATH
   fi
}
