...
function enrollCAAdmin {
   ...
   fabric-ca-client enroll -d -u https://$INT_CA_ADMIN_USER_PASS@$INT_CA_HOST:7054
}
...
function getCACerts {
   ...
   for ORG in $ORGS; do
      ...
      export FABRIC_CA_CLIENT_TLS_CERTFILES=$INT_CA_CHAINFILE
      fabric-ca-client getcacert -d -u https://$INT_CA_HOST:7054 -M $ORG_MSP_DIR
      # Create the tlscacerts and tlsintermediatecerts to mirror the non-tls
      mkdir $ORG_MSP_DIR/tlscacerts $ORG_MSP_DIR/tlsintermediatecerts
