...
# Enroll as the intermediate CA admin
function enrollCA {
   waitForFile $INT_CA_CHAINFILE
   CA=$INT_CA_NAME
   log "Enrolling with $CA as bootstrap identity ..."
   export FABRIC_CA_CLIENT_HOME=$HOME/cas/$CA
   export FABRIC_CA_CLIENT_TLS_CERTFILES=$INT_CA_CHAINFILE
   fabric-ca-client enroll -u https://$INT_CA_ADMIN_USER_PASS@$INT_CA_HOST:7054
}
...
function registerOrdererIdentities {
   ...
   for ORG in $ORDERER_ORGS; do
      ...
      while [[ "$COUNT" -le $NUM_ORDERERS ]]; do
         ...
         log "Registering $ORDERER_NAME with $CA"
         fabric-ca-client register --id.name $ORDERER_NAME --id.secret $ORDERER_PASS
         COUNT=$((COUNT+1))
      done
      log "Registering admin identity with $CA"
      # The admin identity has the "hf.admin" attribute which is added to ECert by default
      fabric-ca-client register --id.name $ADMIN_NAME --id.secret $ADMIN_PASS --id.attrs "hf.admin=true:ecert"
   done
}
...
function registerPeerIdentities {
   ...
   for ORG in $PEER_ORGS; do
      ...
      while [[ "$COUNT" -le $NUM_PEERS ]]; do
         ...
         log "Registering $PEER_NAME with $CA"
         fabric-ca-client register --id.name $PEER_NAME --id.secret $PEER_PASS
         COUNT=$((COUNT+1))
      done
      log "Registering admin identity with $CA"
      # The admin identity has the "hf.admin" attribute which is added to ECert by default
      fabric-ca-client register --id.name $ADMIN_NAME --id.secret $ADMIN_PASS --id.attrs "hf.admin=true:ecert"
      log "Registering user identity with $CA"
      fabric-ca-client register --id.name $USER_NAME --id.secret $USER_PASS
   done
}
...
function getCACerts {
   ...
   for ORG in $ORGS; do
      initOrgVars $ORG
      enrollAdmin
      orgDir=/$SHAREDIR/orgs/$ORG
      mspDir=${orgDir}/msp
      log "Getting CA certs for organization $ORG to $orgDir"
      fabric-ca-client getcacert -M $mspDir
      if [ $ADMINCERTS ]; then
         adminCertsDir=${mspDir}/admincerts
         mkdir -p $adminCertsDir
         cp $CORE_PEER_MSPCONFIGPATH/signcerts/* $adminCertsDir
      fi
   done
}
...
"
   ...
   cp /etc/hyperledger/fabric/configtx.yaml /share
}
