...
function writeSetupFabric {
   echo "  setup:
    ...
    command: /bin/bash -c '/scripts/setup-fabric.sh 2>&1 | tee /$SHAREDIR/logs/setup.log; sleep 99999'
    volumes:
      - ./scripts:/scripts
      - ./$SHAREDIR:/$SHAREDIR
    networks:
...
function writeRunFabric {
   echo "  run:
    container_name: run
    image: hyperledger/fabric-ca-tools
    command: /bin/bash -c 'sleep 3;/scripts/run-fabric.sh 2>&1 | tee /$SHAREDIR/logs/run.log; sleep 99999'
    volumes:
      - ./scripts:/scripts
      - ./$SHAREDIR:/$SHAREDIR
    networks:
...
function writeRootCA {
   echo "  $ROOT_CA_NAME:
    ...
    command: /bin/bash -c '/scripts/start-root-ca.sh $ORG 2>&1 | tee /$SHAREDIR/logs/${ROOT_CA_NAME}.log'
    volumes:
      - ./scripts:/scripts
      - ./$SHAREDIR:/$SHAREDIR
    networks:
...
function writeIntermediateCA {
   echo "  $INT_CA_NAME:
    ...
    image: hyperledger/fabric-ca
    command: /bin/bash -c '/scripts/start-intermediate-ca.sh $ORG 2>&1 | tee /$SHAREDIR/logs/${INT_CA_NAME}.log'
    volumes:
      - ./scripts:/scripts
      - ./$SHAREDIR:/$SHAREDIR
    networks:
...
function writeOrderer {
   echo "  $ORDERER_NAME:
    ...
    command: /bin/bash -c '/scripts/start-orderer.sh $ORG $COUNT 2>&1 | tee /$SHAREDIR/logs/${ORDERER_NAME}.log'
    volumes:
      - ./scripts:/scripts
      - ./$SHAREDIR:/$SHAREDIR
    networks:
...
function writePeer {
   echo "  $PEER_NAME:
    ...
    command: /bin/bash -c '/scripts/start-peer.sh $ORG $COUNT 2>&1 | tee /$SHAREDIR/logs/${PEER_NAME}.log'
    volumes:
      - ./scripts:/scripts
      - ./$SHAREDIR:/$SHAREDIR
    networks:
