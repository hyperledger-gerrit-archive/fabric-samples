...
function writeRootCA {
   echo "  $ROOT_CA_NAME:
    ...
    command: /bin/bash -c '/scripts/start-root-ca.sh 2>&1 | tee $LOGDIR/${ROOT_CA_NAME}.log'
    environment:
...
function writeOrderer {
   ...
   echo "  $ORDERER_NAME:
    ...
    environment:
      ...
      - FABRIC_CA_CLIENT_TLS_CERTFILES=$INT_CA_CHAINFILE
      - ENROLLMENT_URL=https://$ORDERER_NAME_PASS@$INT_CA_HOST:7054
      - ORDERER_HOME=$MYHOME
      ...
      - ORDERER_GENERAL_TLS_ROOTCAS=[$INT_CA_CHAINFILE]
      - ORDERER_GENERAL_LOGLEVEL=debug
      - ORDERER_DEBUG_BROADCASTTRACEDIR=$LOGDIR
      - ORG=$ORG
      - ORG_ADMIN_CERT=$ORG_ADMIN_CERT
    command: /bin/bash -c '/scripts/start-orderer.sh 2>&1 | tee $LOGDIR/${ORDERER_NAME}.log'
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
    networks:
...
function writePeer {
   ...
   echo "  $PEER_NAME:
    ...
    environment:
      - FABRIC_CA_CLIENT_HOME=$MYHOME
      - FABRIC_CA_CLIENT_TLS_CERTFILES=$INT_CA_CHAINFILE
      - ENROLLMENT_URL=https://$PEER_NAME_PASS@$INT_CA_HOST:7054
      - PEER_HOME=$MYHOME
      - PEER_HOST=$PEER_HOST
      ...
      - CORE_PEER_TLS_ROOTCERT_FILE=$INT_CA_CHAINFILE
   ...
   echo "    working_dir: $MYHOME
    command: /bin/bash -c '/scripts/start-peer.sh 2>&1 | tee $LOGDIR/${PEER_NAME}.log'
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
