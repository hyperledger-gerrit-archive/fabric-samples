...
/*
 * Called when chaincode is instantiated, does nothing
 */
...
/*
 * Routes invocations to the appropriate function in chaincode
...
func (s *SmartContract) update(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var txid string = APIstub.GetTxID()
	var compositeIndexName string = "varName~op~value~txID"
...
func (s *SmartContract) pruneSafe(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var valueStr string = string(getResp.Payload)
