...
between when the transaction is simulated on the peer (i.e. read-set is created) and it's ready to be committed by the orderer, another transaction
...
To address this issue, they adopt a bigdata design for the chaincode data model instead. Each sensor has a key which identifies it within the
...
strategies to accommodate bigdata model design.
