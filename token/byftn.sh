...
function printHelp() {
  echo "Usage: "
  echo "  byftn.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-l <language>] [-o <consensus-type>] [-i <imagetag>] [-v]"
  ...
  echo "    -v - verbose mode"
  echo "  byftn.sh -h (print this message)"
  echo
  echo "Typically, one would first generate the required certificates and "
  echo "genesis block, then bring up the network. e.g.:"
  echo
  echo "	byftn.sh generate -c mychannel"
  echo "	byftn.sh up -c mychannel -s couchdb"
  echo "        byftn.sh up -c mychannel -s couchdb -i 1.4.0"
  echo "	byftn.sh up -l node"
  echo "	byftn.sh down -c mychannel"
  echo "        byftn.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	byftn.sh generate"
  echo "	byftn.sh up"
  echo "	byftn.sh down"
}
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of BYFTN and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of BYFTN and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi
  done
}
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile TwoOrgsOrdererGenesis -channelID byftn-sys-channel -outputBlock ./channel-artifacts/genesis.block
  elif [ "$CONSENSUS_TYPE" == "kafka" ]; then
    configtxgen -profile SampleDevModeKafka -channelID byftn-sys-channel -outputBlock ./channel-artifacts/genesis.block
  else
    set +x
