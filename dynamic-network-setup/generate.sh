...
# Generate crypto-config.yaml file from crypto-config-template.yaml template
   ...
   {
    i=1
    sed -n -e '1,23p'  crypto-config-template.yaml > crypto-config.yaml
    while [ "$i" -le "$no_of_orgs" ]
     do
      sed -n -e '24,29p'  crypto-config-template.yaml > crypto-config-temp-file.yaml
...
# Generate configtx.yaml file from configtx-template.yaml template 
   generateConfigTx()
   {
       sed -n -e '1,25p'  configtx-template.yaml > configtx.yaml
	   j=2
       while [ "$j" -le "$no_of_orgs" ]
       do
        sed -n -e '25,25p'  configtx-template.yaml > configtx-template2.yaml
        sed -i "s/*Org1/*Org$j/g" configtx-template2.yaml
        cat < configtx-template2.yaml >> configtx.yaml
        j=$((j + 1))
       done
       sed -n -e '26,31p'  configtx-template.yaml > configtx-template2.yaml
       cat < configtx-template2.yaml >> configtx.yaml
	   j=2
       while [ "$j" -le "$no_of_orgs" ]
       do
        sed -n -e '31,31p'  configtx-template.yaml > configtx-template2.yaml
        sed -i "s/*Org1/*Org$j/g" configtx-template2.yaml
        cat < configtx-template2.yaml >> configtx.yaml
        j=$((j + 1))
       done
       sed -n -e '32,55p'  configtx-template.yaml > configtx-template2.yaml
       cat < configtx-template2.yaml >> configtx.yaml
       j=1
       while [ "$j" -le "$no_of_orgs" ]
       do
        sed -n -e '56,71p'  configtx-template.yaml > configtx-template2.yaml
      ...
      sed -n -e '72,129p'  configtx-template.yaml > configtx-template2.yaml
      cat < configtx-template2.yaml >> configtx.yaml
...
# Generate docker-compose-base.yaml file from docker-compose-base-template.yaml
    generateDockerComposeBase()
	{
	  ...
	  i=1
	  arrayIndex=0
	  sed -n -e '1,29p'  docker-compose-base-template.yaml > artifacts/base/docker-compose-base.yaml
	  while [ "$i" -le "$no_of_orgs" ]
      do
		...
		do
		  echo "peer"
		  echo "$((count-1))"
		  sed -n -e '30,46p'  docker-compose-base-template.yaml > docker-compose-base-template2.yaml
...
# Generate docker-compose.yaml file from docker-compose-e2e-template.yaml
	...
	{
	  sed -n -e '1,6p'  docker-compose-e2e-template.yaml > docker-compose.yaml
	  i=1
	  while [ "$i" -le "$no_of_orgs" ]
      do
        sed -n -e '7,23p'  docker-compose-e2e-template.yaml > docker-compose-e2e-template2.yaml
	  ...
	  sed -n -e '24,31p'  docker-compose-e2e-template.yaml > docker-compose-e2e-template2.yaml
	  cat < docker-compose-e2e-template2.yaml >> docker-compose.yaml
      ...
      do
        ...
        while [ "$count" -le "${peerArray[$arrayIndex]}" ]
         do
           sed -n -e '32,38p'  docker-compose-e2e-template.yaml > docker-compose-e2e-template2.yaml
