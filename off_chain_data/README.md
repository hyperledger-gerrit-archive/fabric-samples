...
to a network created using the `first-network` sample using the Node SDK.
...
NOTE:  Leave the blockEventListener.js running in a window, this will capture block
events to the log file and to CouchDB.  Open a new window to execute the next parts of the demo.
...
Open a new window and run the following command to add 20 marbles to the blockchain:
...
This section of the demo assumes that marbles have been added and the blockEventListener.js 
is running and capturing events.  If the blockEventListener.js is not running, the database
commands below may fail since the database is created based on the received events.
