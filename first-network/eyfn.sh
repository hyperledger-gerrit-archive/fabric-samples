...
function printHelp () {
  ...
  echo "    -l <language> - the programming language of the chaincode to deploy: golang (default), javascript, or java"
...
function networkUp () {
  ...
  echo "###############################################################"
  docker exec cli ./scripts/step3org3.sh $CHANNEL_NAME $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
