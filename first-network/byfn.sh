...
function upgradeNetwork () {
  # removing the backed up from the previous upgrade
  if [ -d orderer.example.com ]; then
  ...
  docker cp -a orderer.example.com:/var/hyperledger/production/orderer ./orderer.example.com/
  ...
  do
    ...
    docker stop $peer
    docker cp -a $peer:/var/hyperledger/production ./$peer/
  ...
  docker exec upgradecli scripts/testupgrade.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Test failed"
    exit 1
  fi
