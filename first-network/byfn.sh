...
function printHelp() {
  ...
  echo "      - 'upgrade'  - upgrade the network from version 1.1.x to 1.2.0"
  ...
  echo "        byfn.sh up -c mychannel -s couchdb -i 1.2.0"
...
# Upgrade the network components which are at version 1.1.x to 1.2.0
...
if [ "${MODE}" == "up" ]; then
  ...
  networkDown
  networkUp
elif [ "${MODE}" == "upgrade" ]; then ## Upgrade the network from version 1.1.x to 1.2.0
  upgradeNetwork
else
  printHelp
  exit 1
