...
function printHelp() {
	...
	echo "      - 'upgrade'  - upgrade the network from v1.1.x to v1.2.x"
...
# Upgrade the network components which are at v1.1.x to v1.2.x
...
if [ "${MODE}" == "up" ]; then
	...
	networkDown
	networkUp
elif [ "${MODE}" == "upgrade" ]; then ## Upgrade the network from v1.1.x to v1.2
	upgradeNetwork
else
	printHelp
	exit 1
