...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ -d data-backup ]; then
      docker run -v $PWD/data-backup:/tmp/data-backup  -w /tmp --name datacleanup -d hyperledger/fabric-tools sleep 60
      docker exec datacleanup rm -rf data-backup/*
      docker rm -f datacleanup
      rm -rf data-backup
    fi
