...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n mycc -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_history >&log.txt
	res=$?
	cat log.txt
...
chaincodeQueryHistory () {
  PEER=$1
  echo "======HISTORY=============== Querying on PEER$PEER on channel '$CHANNEL_NAME'... =========HISTORY============ "
  setGlobals $PEER
  local rc=1
  local starttime=$(date +%s)
  ...
  sleep 3
  echo "Attempting to Query PEER$PEER ...$(($(date +%s)-starttime)) secs"
  peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["queryHistory","a"]}' >&log.txt
  sleep 5  
  echo
  cat log.txt
  if cat log.txt | grep -q "Query Result" ; then
	echo "=========HISTORY============ Query on PEER$PEER on channel '$CHANNEL_NAME' is successful ==========HISTORY=========== "
  else
	echo "!!!!!!!!!!!!!!! Query result on PEER$PEER is INVALID !!!!!!!!!!!!!!!!"
        echo "================== ERROR !!! FAILED to execute End-2-End Scenario =================="
	echo
...
#Query on chaincode on Peer3/Org2, check if the result is 90
echo "Querying chaincode HISTORY on org2/peer3..."
chaincodeQueryHistory 3 0
