...
# parsePeerConnectionParameters <cmdName> $@
# Helper function that takes a command name (e.g. Invoke, Install, Query) and
# all other parameters, checks for an even number of peers and associated org,
# then sets $PEERCONNECTIONPARAMETERS and $PEERS
parsePeerConnectionParameters() {
    cmdName=$1; shift

    if [ $(( $# % 2 )) -ne 0 ]; then
        echo "===================== $cmdName transaction failed on channel '$CHANNEL_NAME' due to uneven number of peer and org parameters ===================== "
        exit 1
    fi
    ...
    while [ "$#" -gt 0 ]; do
        peer="peer$1.org$2"
        peers="$peers $peer"
        PEERCONNECTIONPARAMETERS="$PEERCONNECTIONPARAMETERS --peerAddresses $peer.example.com:7051"
	    if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "true" ]; then
         tlsInfo=$(eval echo "--tlsRootCertFiles \$PEER$1_ORG$2_CA")
         PEERCONNECTIONPARAMETERS="$PEERCONNECTIONPARAMETERS $tlsInfo"
        fi
        shift; shift
    done
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc $PEERCONNECTIONPARAMETERS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc $PEERCONNECTIONPARAMETERS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
