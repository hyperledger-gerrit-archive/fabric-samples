...
packageChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode package mycc.tar.gz --path ${CC_SRC_PATH} --lang ${LANGUAGE} --label mycc_${VERSION}
  res=$?
  set +x
  cat log.txt
...
queryStatus() {
  ...
  do
    ...
    set -x
    peer lifecycle chaincode queryapprovalstatus -o orderer.example.com:7050 --channelID $CHANNEL_NAME --name mycc $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
