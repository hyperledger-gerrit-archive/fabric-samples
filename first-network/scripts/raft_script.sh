...
echo
echo "Build your Raft end-to-end test"
echo "This demo creates and tests a new application channel with 3 orderers (subset of the 5 orderers used in BYFN)"
echo "Once the new channel is created, the same script as BYFN is run"
echo
CHANNEL_NAME="$1"
...
createChannel() {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer channel create -o orderer.example.com:7050 -c $CHANNEL_NAME -f "$1" >&log.txt
		res=$?
                set +x
	else
				set -x
		peer channel create -o orderer.example.com:7050 -c $CHANNEL_NAME -f "$1" --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
		res=$?
				set +x
	fi
	cat log.txt
...
createChannel "$CREATE_CHANNEL_FILE"
...
echo
echo "========= All GOOD, Raft execution completed =========== "
echo
