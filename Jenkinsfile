...
//    env.VERSION = "1.3.0"  // Fabric Released Version from release-1.3 branch
    env.VERSION = sh(script: "curl -L https://raw.githubusercontent.com/hyperledger/fabric/release-1.3/Makefile" > Makefile && cat Makefile | grep "PREV_VERSION =" | cut -d "=" -f2, returnStatus: true)
    env.BASE_IMAGE_VER = sh(script: cat Makefile | grep BASEIMAGE_RELEASE= | cut -d "=" -f2, returnStatus: true)
