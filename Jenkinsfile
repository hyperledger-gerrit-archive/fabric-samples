...
node ('hyp-x') { // trigger build on x86_64 node
  timestamps {
   try {
    ...
    env.VERSION = sh(script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/release-1.2/Makefile && cat Makefile | grep "PREV_VERSION =" | cut -d "=" -f2', returnStatus: true)
