...
node ('hyp-x') { // trigger build on x86_64 node
  timestamps {
   try {
    ...
    env.OS_VER = "amd64"
    env.VERSION = sh(script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/release-1.3/Makefile && cat Makefile | grep "PREV_VERSION =" | cut -d "=" -f2 | tr -d '[:space:]'', returnStdout: true)
    env.VERSION = "$VERSION"
