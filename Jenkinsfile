  ...
  pipeline {
    agent {
      ...
      environment {
        ...
        // Send an email to helpdesk@hyperledger.org to add a newer version
          ...
          steps {
            script {
                if(props["IMAGE_SOURCE"] == "build") {
                  ...
                  // Copy binaries into fabric-samples dir
                  sh 'cp -r $ROOTDIR/gopath/src/github.com/hyperledger/fabric/release/$ARCH-$MARCH/bin $ROOTDIR/$BASE_DIR/'
                  ...
                  dir("$ROOTDIR/$BASE_DIR") {
                    ...
                    env.GOROOT = "/opt/go/go" + props["GO_VER"] + ".linux." + "$MARCH"
                    env.PATH = "$GOPATH/bin:$GOROOT/bin:/usr/local/bin:/usr/bin:/usr/local/sbin:/usr/sbin:${nodeHome}/bin:$PATH"
                    // Pull Binaries with latest version from nexus2 snapshot url
                    // https://nexus.hyperledger.org/content/repositories/snapshots/org/hyperledger/fabric/hyperledger-fabric-latest/
        ...
        failure {
          script {
            if (env.JOB_TYPE == 'merge') {
              ...
              // clean the workspace after build done
