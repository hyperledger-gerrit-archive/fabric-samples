...
node ('hyp-x') { // trigger build on x86_64 node
  timestamps {
   try {
    ...
    env.ARCH = "amd64"
    env.VERSION = sh(returnStdout: true, script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/release-1.1/Makefile && cat Makefile | grep "PREV_VERSION =" | cut -d "=" -f2').trim()
