...
node ('hyp-x') { // trigger build on x86_64 node
  timestamps {
   try {
    ...
    env.OS_VER = sh(script: 'dpkg --print-architecture', returnStdout: true)
    env.VERSION = sh(script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/release-1.3/Makefile && cat Makefile | grep "PREV_VERSION =" | cut -d "=" -f2', returnStdout: true)
    env.BASE_IMAGE_VER = sh(script: 'cat Makefile | grep BASEIMAGE_RELEASE= | cut -d "=" -f2', returnStdout: true)
    env.BASE_IMAGE_TAG = sh(script: '$OS_VER-$BASE_IMAGE_VER', returnStdout: true)
