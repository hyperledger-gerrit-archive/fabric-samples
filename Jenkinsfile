...
node ('hyp-x') { // trigger build on x86_64 node
  timestamps {
   try {
    ...
    env.ARCH = "x86_64"
    env.VERSION = sh(returnStdout: true, script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/release-1.0/Makefile && cat Makefile | grep "PREV_VERSION =" | cut -d "=" -f2').trim()
    ...
    env.BASE_IMAGE_TAG = "${ARCH}-1.0.6" //fabric baseimage version
    ...
    // Pull Third Party Images (couchdb, zookeeper, kafka)
      ...
      // Run byfn tests (default, custom channel, custom channel-couchdb)
      stage("Run byfn Tests") {
         // make the output color coded
         wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
           try {
                 dir("${ROOTDIR}/$PROJECT_DIR/fabric-samples/scripts/Jenkins_Scripts") {
                 sh './CI_Script.sh --byfn_Tests'
                 }
               }
           catch (err) {
                 failure_stage = "byfn_Tests"
                 currentBuild.result = 'FAILURE'
                 throw err
           }
         }
