...
Next, use the FabToken application to interact with the Fabric network.
  ...
  Then run the fabtoken application to perform the token operations.
    ...
    node fabtoken issue <username> <token_type> <quantity>
      ...
      - issue a token to user1 in with type of "USD" and quantity of 100
    ...
    node fabtoken transfer <from_user> <to_user> <transfer_quantity> <remaining_quantity> <tx_id_from_list> <index_from_list>
      - example: node fabtoken transfer user1 user2 30 70 c9b1211d9ad809e6ee1b542de6886d8d1d9e1c938d88eff23a3ddb4e8c080e4d 0
      - user1 transfers to user2 "30" quantity of the token as specified by <tx_id_from_list> and <index_from_list>
      - <tx_id_from_list> and <index_from_list> are the "tx_id" and "index" returned from the list operation
    ...
    node fabtoken redeem <username> <redeem_quantity> <tx_id_from_list> <index_from_list>
      - example: node fabtoken redeem user2 10 477c7bf2002814497c228fd8cbc4d80c8b7f1602b2c17ffadb6cf7e5783fa47a 0
      - user2 redeems "10" quantity of the token as specified by <tx_id_from_list> and <index_from_list>
      - <tx_id_from_list> and <index_from_list> are the "tx_id" and "index" returned from the list operation
