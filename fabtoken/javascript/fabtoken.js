	...
	try {
		...
		switch (operation) {
			...
			case 'transfer':
				...
				if (args.length < 5) {
					throw new Error('Missing required parameters for transfer: recipient, transfer_quantity, remaining_quantity, tx_id, index');
				}
				let recipient
				if (args[0] === 'user1') {
					recipient = user1;
		...
		console.log('End token operation %s, returns %s', operation, util.inspect(result, {depth: null}));
...
async function issue(client, channel, admin, user, args) {
	...
	const param = {
		owner: {raw: user.getIdentity().serialize(), type: 0},
		type: args[0],
		quantity: args[1]
	};
	const request = {
...
// Transfers token from the "user" to the "recipient"
async function transfer(client, channel, user, recipient, args) {
	...
	// build the request for "user" to transfer tokens to "recipient"
	const txId = client.newTransactionID();
	const param1 = {
		owner: {raw: recipient.getIdentity().serialize(), type: 0},
		quantity: args[1]
	};
	let request = null;
	if (args[2] === '0') {
...
async function redeem(client, channel, user, args) {
	...
	// build the request for "user" to redeem tokens
	const txId = client.newTransactionID();
	const param = {
		quantity: args[0]
	};
