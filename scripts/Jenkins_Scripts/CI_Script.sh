...
pull_Docker_Images() {
            ...
            for IMAGES in peer orderer tools ccenv; do
                 echo "-----------> pull $IMAGES image"
                 echo
                 docker pull $NEXUS_URL/$ORG_NAME-$IMAGES:$PROJECT_VERSION > /dev/null 2>&1
                 if [ $? -ne 0 ]; then
                       echo -e "\033[31m FAILED to pull docker images" "\033[0m"
                       exit 1
                 fi
                 docker tag $NEXUS_URL/$ORG_NAME-$IMAGES:$PROJECT_VERSION $ORG_NAME-$IMAGES
                 docker tag $NEXUS_URL/$ORG_NAME-$IMAGES:$PROJECT_VERSION $ORG_NAME-$IMAGES:$PROJECT_VERSION
                 docker rmi -f $NEXUS_URL/$ORG_NAME-$IMAGES:$PROJECT_VERSION
            done
                 echo
                 docker images | grep hyperledger/fabric
}
