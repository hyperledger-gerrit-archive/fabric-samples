...
do
    ...
    if docker exec cli3ecobank ls > /dev/null 2>&1
    then
...
if ! docker exec cli3ecobank peer channel getinfo -c papernet
then
    docker exec cli3ecobank peer channel fetch 0 -o orderer1.digibank.example.com:7050 -c papernet papernet.block
    docker exec cli3ecobank peer channel join -b papernet.block
fi
