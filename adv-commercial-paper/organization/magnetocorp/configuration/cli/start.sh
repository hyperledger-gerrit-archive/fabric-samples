...
do
    ...
    if docker exec cli7magnetocorp ls > /dev/null 2>&1
    then
...
if ! docker exec cli7magnetocorp peer channel getinfo -c papernet
then
    docker exec cli7magnetocorp peer channel fetch 0 -o orderer1.digibank.example.com:7050 -c papernet papernet.block
    docker exec cli7magnetocorp peer channel join -b papernet.block
fi
